...
public:
	...
	void show_stats()
	{
		...
		if (landing_queue->get_num_served() > 0)
			std::cout << landing_queue->get_total_wait() / landing_queue->get_num_served() << std::endl;
		else
			std::cout << landing_queue->get_total_wait();
		...
		std::cout << "The average wait time for the departure queue: ";
		if (departure_queue->get_num_served() > 0)
			std::cout << departure_queue->get_total_wait() / departure_queue->get_num_served() << std::endl;
		else
			std::cout << departure_queue->get_total_wait();
