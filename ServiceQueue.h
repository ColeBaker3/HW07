...
public:
	...
	void set_number_of_gates(int num) {
		this->num_of_gates = num;
	}
	...
	void update(int clock)
	{
			...
			if (the_queue.empty()) {
				...
				if (!landing_queue->the_queue.empty()) {
					...
					int landing_queue_wait_time = (clock - plane->arrival_time);

					// FIXME: update total_wait and num_served for the landing queue
					landing_queue->num_served++;
					landing_queue->total_wait = +landing_queue_wait_time;
